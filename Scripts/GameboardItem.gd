...
#func get_corner_positions():
	...
	#
	#return {"top_left":     Vector2(position.x - w/2, position.y - h/2), 
			#"top_right":    Vector2(position.x + w/2, position.y - h/2), 
			#"bottom_left":  Vector2(position.x - w/2, position.y + h/2),
			#"bottom_right": Vector2(position.x + w/2, position.y + h/2)}
	#
#
#
#
